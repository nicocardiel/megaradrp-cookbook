...
1. INTRODUCTION `8 <#introduction>`__

1.1 Scope `8 <#scope>`__

1.2 MEGARA instrument `8 <#megara-instrument>`__

2. MEGARA Data Reduction Pipeline
`11 <#megara-data-reduction-pipeline>`__

3. DRP INSTALLATION `12 <#drp-installation>`__

3.1 Install in virtualenv `12 <#install-in-virtualenv>`__

3.1.1 Create a virtual environment using either virtualenv or venv.
`12 <#create-a-virtual-environment-using-either-virtualenv-or-venv.>`__

3.1.2 Activate the environment. `12 <#activate-the-environment.>`__

3.1.3 Install megaradrp with pip `13 <#install-megaradrp-with-pip>`__

3.1.4 Test the installation. `13 <#test-the-installation.>`__

3.1.5 Update within the environment
`14 <#update-within-the-environment>`__

3.1.6 Deactivate the environment. `14 <#deactivate-the-environment.>`__

3.2 Install in conda `14 <#install-in-conda>`__

3.2.1 Create a conda environment `15 <#create-a-conda-environment>`__

3.2.2 Install megaradrp with conda
`15 <#install-megaradrp-with-conda>`__

3.2.3 Activate the environment `16 <#activate-the-environment>`__

3.2.4 Test the installation `16 <#test-the-installation>`__

3.2.5 Update within the environment
`16 <#update-within-the-environment-1>`__

3.2.6 Deactivate the environment `16 <#deactivate-the-environment>`__

3.2.7 Update outside the environment
`16 <#update-outside-the-environment>`__

3.3 Development version `16 <#development-version>`__

4. DATA DESCRIPTION `17 <#data-description>`__

4.1 Raw Data `17 <#raw-data>`__

4.2 Pipeline Products `17 <#pipeline-products>`__

5. DATA REDUCTION COOKBOOK `18 <#data-reduction-cookbook>`__

5.1 Getting Started `18 <#getting-started>`__

5.2 Data organization `18 <#data-organization>`__

5.3 Running a recipe `22 <#_Toc14254698>`__

5.4 Data reduction process `24 <#data-reduction-process>`__

5.4.1 Bias image `24 <#bias-image>`__

5.4.2 Dark image `25 <#dark-image>`__

5.4.3 Bad-pixels Mask `25 <#bad-pixels-mask>`__

5.4.4 Slit Flat correction `26 <#slit-flat-correction>`__

5.4.5 Tracing fibers `26 <#tracing-fibers>`__

5.4.5.1 Trace map `26 <#trace-map>`__

5.4.5.2 Model map `28 <#model-map>`__

5.4.6 Wavelength Calibration `29 <#wavelength-calibration>`__

5.4.7 Flat-field correction `33 <#flat-field-correction>`__

5.4.8 Illumination correction `35 <#illumination-correction>`__

5.4.9 Flux calibration `37 <#flux-calibration>`__

5.4.10 LCB IFU/MOS scientific observation
`41 <#lcb-ifumos-scientific-observation>`__

6. Known ISSUES `45 <#known-issues>`__

6.1 Matplotlib 3 in MacOSX: `45 <#matplotlib-3-in-macosx>`__

6.2 Compiling in MacOSX Mojave: `46 <#compiling-in-macosx-mojave>`__
...
in Table 1. This table also includes the reciprocal (linear) disperision
...
this calibration tree according to the following structure:
...
day but at different ambient temperatures. Different files can be stored
at the same directory, but the DRP is going to use the first file it
encounters in alphabetical order. The user can name the desired file
with prefix *“00\_”* (e.g. *00_master_traces.json*) to be sure this is
the file to be used by the DRP. Note that the sorting of files named
*“00\_”* and *“000\_”* might be different for the operative system and
...
should be evaluated and computed as detailed in last section 5.4.6.
...
default, the DRP searches for these data files in the data/ directory.
...
**Figure 15:** *Example of the final_rss.fits (sky subtracted,
wavelength and flux calibrated) file for object M15 in the HR-R setup
and the LCB observing mode.*
...
visualization.py code included in the MEGARA DRP distribution.*

The *final_rss.fits* image (or, equivalently, the *reduced_rss.fits*
image) obtained after running the MegaraLcbImage processing recipe can
be visualized using the visualization.py code distributed with the
MEGARA DRP. The way to execute this code is to use the following command
under the corresponding MEGARA environment:

(megara) bash-3.2$ python -m megaradrp.visualization final_rss.fits
--label "flux (Jy)" --wcs-grid --title "M32 (flux)"
...
Matplotlib 3 in MacOSX:
-----------------------

In the case of using matplotlib 3 under MacOSX (installations starting
...
Note that TkAgg is the default backend in MacOSX. If you want to use
alternative ones they also need to be installed, or course.

Compiling in MacOSX Mojave:
---------------------------

Should the user be insterested in compiling part of the software on
their own, we note that in the case of the Mojave distribution of MacOSX
they should include the following environment variable in the
$HOME/.bashrc shell configuration file:
...
.. |image11| image:: _static/image21.png
   :width: 6.69306in
   :height: 1.27917in
.. |image12| image:: _static/image23.png
   :width: 6.69306in
   :height: 1.33889in
